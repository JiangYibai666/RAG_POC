...
from agents.market_agent.graph import run_market_graph
...
async def stream_handler(request: TaskRequest) -> AsyncIterator[TaskEvent]:
    ...
    yield TaskEvent(task_id=request.task_id, state=TaskState.WORKING, message="MarketAgent: retrieving market context")
    await asyncio.sleep(0.05)

    artifact = run_market_graph(request.message)
    add_artifact(request.task_id, artifact)
    update_task_state(request.task_id, TaskState.COMPLETED)

    yield TaskEvent(task_id=request.task_id, state=TaskState.COMPLETED, message="MarketAgent: completed", artifact=artifact)


def create_app() -> FastAPI:
    _app = FastAPI(title="MarketAgent", version="0.1.0")
